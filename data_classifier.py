...
class DataBowl3Classifier(Dataset):
    def __init__(self, split, config, phase='train'):
        ...
        self.filenames = [os.path.join(datadir, '%s_clean.npy' % idx) for idx in
                          idcs]
        if self.phase != 'test':
            self.yset = 1 - np.array([f.split('-')[1][2] for f in idcs]).astype(
                'int')
    ...
    def __getitem__(self, idx, split=None):
        ...
        if self.phase != 'test':
            ...
            return torch.from_numpy(croplist).float(), torch.from_numpy(
                coordlist).float(), torch.from_numpy(
...
class simpleCrop():
    ...
    def __call__(self, imgs, target):
        ...
        self.used_crop_rect = {
            "z": [float(start[0] - pad[1][0]) / org_shape[1],
                  float(start[0] + crop_size[0] - pad[1][0]) / org_shape[1]],
            "y": [float(start[1] - pad[2][0]) / org_shape[2],
                  float(start[1] + crop_size[1] - pad[2][0]) / org_shape[2]],
            "x": [float(start[2] - pad[3][0]) / org_shape[3],
                  float(start[2] + crop_size[2] - pad[3][0]) / org_shape[3]],
        }
...
def sampleone(target, conf, T):
    ...
    p = p / np.sum(p)
    return np.random.choice(np.arange(len(target)), size=1, replace=False, p=p)[
        0]
...
def augment(sample, coord, ifflip=True, ifrotate=True, ifswap=True,
    ...
    if ifflip:
        flipid = np.array([np.random.randint(2), np.random.randint(2),
