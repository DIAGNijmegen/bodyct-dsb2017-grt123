...
class DataBowl3Detector(Dataset):
    def __init__(self, split, config, phase='train', split_comber=None):
        ...
        if self.channel == 2:
            self.filenames = [os.path.join(data_dir, '%s_merge.npy' % idx) for
                              idx in idcs]
        elif self.channel == 1:
            if 'cleanimg' in config and config['cleanimg']:
                ...
                self.filenames = [os.path.join(data_dir, '%s_img.npy' % idx) for
                                  idx in idcs]
        self.kagglenames = [f for f in self.filenames if
                            len(f.split('/')[-1].split('_')[0]) > 20]
        self.lunanames = [f for f in self.filenames if
                          len(f.split('/')[-1].split('_')[0]) < 20]
        ...
        for idx in idcs:
            if config['luna_raw'] == True:
                try:
                    l = np.load(
                        os.path.join(data_dir, '%s_label_raw.npy' % idx))
                except:
    ...
    def __getitem__(self, idx, split=None):
        ...
        if self.phase != 'test':
            if not isRandomImg:
                ...
                sample, target, bboxes, coord = self.crop(imgs, bbox[1:],
                                                          bboxes, isScale,
                                                          isRandom)
                if self.phase == 'train' and not isRandom:
                    sample, target, bboxes, coord = augment(sample, target,
                                                            bboxes, coord,
                                                            ifflip=self.augtype[
                                                                'flip'],
                                                            ifrotate=
                ...
                isScale = self.augtype['scale'] and (self.phase == 'train')
                sample, target, bboxes, coord = self.crop(imgs, [], bboxes,
                                                          isScale=False,
                                                          isRand=True)
            ...
            return torch.from_numpy(
                imgs.astype(np.float32)), bboxes, torch.from_numpy(
...
def augment(sample, target, bboxes, coord, ifflip=True, ifrotate=True,
            ifswap=True):
    ...
    if ifrotate:
        ...
        while not validrot:
            ...
            if np.all(newtarget[:3] > target[3]) and np.all(
                    newtarget[:3] < np.array(sample.shape[1:4]) - newtarget[3]):
                validrot = True
                target = newtarget
                ...
                if counter == 3:
                    break
    if ifswap:
        if sample.shape[1] == sample.shape[2] and sample.shape[1] == \
                sample.shape[3]:
...
class Crop(object):
    ...
    def __call__(self, imgs, target, bboxes, isScale=False, isRand=False):
        if isScale:
            ...
            scaleLim = [0.75, 1.25]
            scaleRange = [
                np.min([np.max([(radiusLim[0] / target[3]), scaleLim[0]]), 1])
                ,
                np.max([np.min([(radiusLim[1] / target[3]), scaleLim[1]]), 1])]
            scale = np.random.rand() * (scaleRange[1] - scaleRange[0]) + \
                    scaleRange[0]
        ...
        for i in range(3):
            if not isRand:
                ...
                e = np.min(
                    [crop_size[i] / 2, imgs.shape[i + 1] / 2 - bound_size])
                target = np.array([np.nan, np.nan, np.nan, np.nan])
            if s > e:
                start.append(np.random.randint(e, s))  # !
            else:
                start.append(
                    int(target[i]) - crop_size[i] / 2 + np.random.randint(
...
class LabelMapping(object):
    ...
    def __call__(self, input_size, target, bboxes):
        ...
        oz = np.arange(offset, offset + stride * (output_size[0] - 1) + 1,
                       stride)
        oh = np.arange(offset, offset + stride * (output_size[1] - 1) + 1,
                       stride)
        ow = np.arange(offset, offset + stride * (output_size[2] - 1) + 1,
                       stride)
        ...
        for bbox in bboxes:
            for i, anchor in enumerate(anchors):
                ...
                label[:, :, :, i, 0] = binary_dilation(
                    label[:, :, :, i, 0].astype('bool'), structure=struct,
...
def select_samples(bbox, anchor, th, oz, oh, ow):
    ...
    if min_overlap > max_overlap:
        return np.zeros((0,), np.int64), np.zeros((0,), np.int64), np.zeros(
            (0,), np.int64)
    else:
