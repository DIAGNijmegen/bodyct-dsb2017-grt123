...
class PostRes(nn.Module):
    def __init__(self, n_in, n_out, stride=1):
        ...
        self.conv1 = nn.Conv3d(n_in, n_out, kernel_size=3, stride=stride,
                               padding=1)
...
class Rec3(nn.Module):
    ...
    def forward(self, x0, x1, x2, x3):
        if self.p > 0 and self.training:
            ...
            out1 = coef[0] * self.block01(x0) + coef[1] * self.block11(x1) + \
                   coef[2] * self.block21(x2)
            out2 = coef[3] * self.block12(x1) + coef[4] * self.block22(x2) + \
                   coef[5] * self.block32(x3)
            out3 = coef[6] * self.block23(x2) + coef[7] * self.block33(x3)
        else:
            out1 = (1 - self.p) * (
                        self.block01(x0) + self.block11(x1) + self.block21(x2))
            out2 = (1 - self.p) * (
                        self.block12(x1) + self.block22(x2) + self.block32(x3))
...
class Loss(nn.Module):
    ...
    def forward(self, output, labels, train=True):
        ...
        if self.num_hard > 0 and train:
            neg_output, neg_labels = hard_mining(neg_output, neg_labels,
                                                 self.num_hard * batch_size)
        ...
        if len(pos_output) > 0:
            ...
            pz, ph, pw, pd = pos_output[:, 1], pos_output[:, 2], pos_output[:,
                                                                 3], pos_output[
                                                                     :, 4]
            lz, lh, lw, ld = pos_labels[:, 1], pos_labels[:, 2], pos_labels[:,
                                                                 3], pos_labels[
                                                                     :, 4]
        ...
        return [loss, classify_loss_data] + regress_losses_data + [pos_correct,
                                                                   pos_total,
                                                                   neg_correct,
                                                                   neg_total]
...
class GetPBB(object):
    ...
    def __call__(self, output, thresh=-3, ismask=False):
        ...
        output[:, :, :, :, 1] = oz.reshape((-1, 1, 1, 1)) + output[:, :, :, :,
                                                            1] * anchors.reshape(
            (1, 1, 1, -1))
        output[:, :, :, :, 2] = oh.reshape((1, -1, 1, 1)) + output[:, :, :, :,
                                                            2] * anchors.reshape(
            (1, 1, 1, -1))
        output[:, :, :, :, 3] = ow.reshape((1, 1, -1, 1)) + output[:, :, :, :,
                                                            3] * anchors.reshape(
            (1, 1, 1, -1))
        output[:, :, :, :, 4] = np.exp(output[:, :, :, :, 4]) * anchors.reshape(
            (1, 1, 1, -1))
...
def iou(box0, box1):
    ...
    union = box0[3] * box0[3] * box0[3] + box1[3] * box1[3] * box1[
        3] - intersection
